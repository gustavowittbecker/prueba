...
Param(
   ...
   [string]$Certificado="micertificado.pem",
...
# PASO 2: FIRMAR CMS
C:\OpenSSL\bin\openssl.exe cms -sign -in $seqNr-$OutXml -signer $Certificado -inkey $ClavePrivada -nodetach -outform der -out $seqNr-$OutCms-DER

# PASO 3: ENCODEAR EL CMS EN BASE 64
C:\OpenSSL\bin\openssl.exe base64 -in $seqNr-$OutCms-DER -e -out $seqNr-$OutCms-DER-b64
